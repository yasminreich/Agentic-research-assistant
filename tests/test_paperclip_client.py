"""Tests for the OpenAlex client (`app/paperclip_client.py`).
...
from app.paperclip_client import MAX_LIMIT, PaperclipClient, PaperclipError
...
def no_real_sleeping(monkeypatch):
    ...
    slept: list[float] = []
    monkeypatch.setattr("app.paperclip_client.time.sleep", slept.append)
    return slept


def make_client(responses, **kwargs):
    return PaperclipClient(mailto="", session=FakeSession(responses), **kwargs)


class TestReconstructAbstract:
    def test_rebuilds_words_in_position_order(self):
        index = {"world": [1], "Hello": [0], "again": [2]}
        assert PaperclipClient._reconstruct_abstract(index) == "Hello world again"

    def test_a_repeated_word_appears_at_every_position(self):
        index = {"the": [0, 2], "cat": [1], "sat": [3]}
        assert PaperclipClient._reconstruct_abstract(index) == "the cat the sat"

    @pytest.mark.parametrize("empty", [None, {}])
    def test_absent_index_yields_none(self, empty):
        assert PaperclipClient._reconstruct_abstract(empty) is None


class TestNormalize:
    def test_maps_a_full_record(self):
        paper = PaperclipClient._normalize(WORK)
    ...
    def test_strips_the_doi_url_prefix(self):
        assert PaperclipClient._normalize(WORK).doi == "10.1016/j.cmet.2018.04.010"

    def test_missing_primary_location_is_survivable(self):
        paper = PaperclipClient._normalize({"id": "W1", "display_name": "T"})
    ...
    def test_null_primary_location_is_survivable(self):
        paper = PaperclipClient._normalize(
    ...
    def test_authorships_without_a_name_are_skipped(self):
        ...
        assert PaperclipClient._normalize(record).authors == ["Ada"]
...
class TestSearch:
    ...
    def test_sends_the_query_and_year_filter(self):
        ...
        PaperclipClient(mailto="", session=session).search("fasting", year_from=2019)
...
class TestRetryBehaviour:
    def test_retries_after_a_429_then_succeeds(self, no_real_sleeping):
        ...
        papers = PaperclipClient(mailto="", session=session).search("q")
        assert len(papers) == 1
    ...
    def test_retries_transient_5xx(self):
        ...
        assert len(PaperclipClient(mailto="", session=session).search("q")) == 1
    ...
    def test_gives_up_after_max_retries(self):
        ...
        client = PaperclipClient(mailto="", session=session, max_retries=2)
        with pytest.raises(PaperclipError, match="after 3 attempts"):
            client.search("q")
        assert len(session.calls) == 3

    def test_client_errors_are_not_retried(self):
        session = FakeSession([FakeResponse(400, text="bad query")])
        with pytest.raises(PaperclipError, match="400"):
            PaperclipClient(mailto="", session=session).search("q")
